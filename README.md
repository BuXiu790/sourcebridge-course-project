...
- 邮箱密码登录、退出和会话保持
- 公开 Buyer 注册、即时登录会话与数据库自动创建 Buyer Profile
...
| `/signup` | 匿名 | Course Demo Buyer 注册；邮箱所有权暂不验证，新用户固定为 Buyer |
| `/forgot-password` | 匿名 | 显示课程版本暂不支持密码恢复，不发送无效重置邮件 |
...
| 业务 API | 9 个 Next.js Route Handler，覆盖注册、RFQ、报价、状态、时间线、供应商和角色 |
...
| 自动化测试 | 18/18 通过，并包含注册校验、生产构建和安全契约 |
...
`npm test` 会先执行生产构建，再运行 18 项自动化测试。`npm run test:live` 验证既有四角色权限矩阵；`npm run test:signup-live` 使用自动生成的临时邮箱验证公开注册、Buyer 默认角色、越权防护、会话恢复和五条 RFQ 限制。两项真实测试均只使用公开客户端参数和普通用户会话，不使用 service role key；临时业务数据自动清理，临时 Auth 用户由 Supabase 控制台删除。
...
- Course v1.1 开放 Buyer 注册，但暂时关闭邮箱确认，因此不验证邮箱所有权
- 密码恢复在配置自定义 SMTP 前不可用；商业化前必须重新启用邮箱确认
- 每个普通 Buyer 最多创建 5 条 RFQ，该限制由服务端和数据库共同强制执行
